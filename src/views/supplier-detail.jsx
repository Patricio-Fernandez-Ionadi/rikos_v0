...
export const SupplierDetailPage = () => {
	...
	const handleAddProduct = useCallback(async () => {
		...
		setPurchaseCost('')
		setSearchTerm('')
	}, [id, addMode, selectedProductId, newName, newCatId, purchaseCost, setProducts])
	...
	const handleUpdateCost = useCallback(async (psId) => {
		...
		if (isNaN(cost) || cost < 0) return
		try {
			const updated = await api.updateProductSupplier(psId, { purchaseCost: cost })
			setProductSuppliers((prev) => prev.map((ps) => (ps._id === updated._id ? updated : ps)))
		} catch (e) {
			console.error(e)
		}
		setEditingPS(null)
	...
	return (
		<div className='stock-page'>
			...
			<div className='detail-page__section-header' style={{ marginBottom: '12px' }}>
				...
				<button className='sidebar__btn' onClick={() => setShowAddForm(!showAddForm)}
			...
			{showAddForm && (
				<div style={{
					...
					<div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
						<button className={`sidebar__btn ${addMode === 'existing' ? 'sidebar__btn--active' : ''}`}
							style={{ width: 'auto', padding: '4px 10px' }}
							onClick={() => setAddMode('existing')}>
							Producto existente
						</button>
						<button className={`sidebar__btn ${addMode === 'new' ? 'sidebar__btn--active' : ''}`}
							style={{ width: 'auto', padding: '4px 10px' }}
					...
					{addMode === 'existing' ? (
						...
						<>
							...
							<select className='field-input' value={newCatId}
								onChange={(e) => setNewCatId(e.target.value)}>
								{categories.map((c) => (
									<option key={c._id} value={c._id}>{c.name}</option>
								))}
							</select>
							<label className='field-label' style={{ marginTop: '6px' }}>Nombre del producto</label>
							<input className='field-input' type='text' value={newName}
								onChange={(e) => setNewName(e.target.value)} />
						</>
					)}

					<label className='field-label' style={{ marginTop: '6px' }}>Precio de costo ($)</label>
					<input className='field-input' type='number' value={purchaseCost}
						onChange={(e) => setPurchaseCost(e.target.value)} />

					<div className='modal-actions' style={{ marginTop: '10px' }}>
						<button className='shift-bar__btn shift-bar__btn--primary'
							disabled={(addMode === 'existing' && !selectedProductId) || (addMode === 'new' && (!newName.trim() || !newCatId)) || !purchaseCost}
							...
							{productSuppliers.map((ps) => {
								...
								return (
									<tr key={ps._id}>
										...
										<td>{category?.name ?? '—'}</td>
										<td style={{ color: '#e0e0e0', cursor: 'pointer' }}
											onClick={() => {
												if (editingPS !== ps._id) {
													setEditingPS(ps._id)
