...
const CSVTOREAD = 'productos-exel_1.csv'
...
function detectPresentationLabel(productName, grams) {
	for (const { regex, fmt } of PRESENTATION_PATTERNS) {
		...
		if (match) {
			return {
				baseName: productName.slice(0, match.index).trim(),
				label: fmt(match),
			}
		}
	}
...
export function migrate(rows) {
	...
	for (const raw of rows) {
		const { catId, categoria, producto, costoUn, gr, margen, venta } =
			parseRow(raw)
...
export function runMigration() {
	...
	if (merged.length > 0) {
		...
		for (const p of merged) {
			...
			for (const pr of preses) {
				console.log(
					`    → ${pr.label} | ${pr.grams != null ? pr.grams + 'g' : 'entero'} | margen ${pr.margin ?? '-'}% | venta $${pr.salePrice ?? '-'}`,
				)
			}
		}
	}
	...
	if (sinCosto.length > 0) {
		...
		for (const p of sinCosto) {
			...
			for (const pr of preses) {
				console.log(
					`  ${p.name} → ${pr.label} | margen ${pr.margin ?? '-'}% | venta $${pr.salePrice ?? '-'}`,
				)
			}
		}
	}
